<script lang="ts">
import { identity, transition_in } from 'svelte/internal';

    import type {ITodo} from '../types/todo'
    type  completeTodoType = (id:string)=>void
    type  RemoveTodoType= (id:string)=>void
    type  EditTodoType= (id:string,content:string)=>void
...
function handleEdit(event:KeyboardEvent,id:string):void{
    ...
    switch(pressedKey){
        ...
        case "Escape":
            targetElement.blur() //when element lose focus
            break;
...
<li class:editing class="todo">
                <div class="todo-item">
                    ...
                    <div>
                    <!-- <input on:change={()=>{todo.completed=!todo.completed}} checked={todo.completed} type="checkbox" id="todo" class="toggle"> -->

                                        <!-- call function inside another function so it can be called everytime we made change -->
                     <input on:change={()=>{completeTodo(todo.id)}} checked={todo.completed} type="checkbox" id="todo" class="toggle"> 
                    <label for="todo" class="todo-check"/>
                    </div>
                ...
                {#if editing}
                    <input on:keydown={(event)=>{handleEdit(event,todo.id)}}
                     on:blur={(event)=>{handleBlur(event,todo.id)}}
                     value={todo.text} type="text" autofocus class="edit">
                {/if}
